...
Restent ensuite : les glyphes de **Parole III** (`lire`, `scribe`, `archive`, `les-lecteurs`) et la branche **Modalité** — d'où `zéro` = `ne-pas` + `un`, qui attend `ne-pas` et sera le premier composé à faire mordre une branche tardive sur la branche Nombre —, l'instrument **Élève**, puis les **contradictions** de l'acte IV. Voir `docs/design-doc.md` §7 et §8, et `docs/backlog-1.0.md` pour le découpage.
